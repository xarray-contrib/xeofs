...
@pytest.mark.parametrize('scaling', [0, 1, 2])
def test_wrapper_multivariate_solutions(scaling, sample_array):
    # Solutions of numpy, pandas and xarray wrapper are the same
    X = sample_array
    X1 = X[:, :10]
    X2 = X[:, 10:]
    ...
    # PCs
    desired_pcs = numpy_model.pcs(scaling=scaling)
    actual_pandas_pcs = pandas_model.pcs(scaling=scaling)
    actual_xarray_pcs = xarray_model.pcs(scaling=scaling)
    # EOFs
    desired_eofs = numpy_model.eofs(scaling=scaling)
    actual_pandas_eofs = pandas_model.eofs(scaling=scaling)
    actual_xarray_eofs = xarray_model.eofs(scaling=scaling)
    # EOFs as correlation
    desired_eofs_corr = numpy_model.eofs_as_correlation()
    actual_pandas_eofs_corr = pandas_model.eofs_as_correlation()
    actual_xarray_eofs_corr = xarray_model.eofs_as_correlation()
    ...
    actual_xarray_Xrec = xarray_model.reconstruct_X()
    # Projection onto EOFs
    desired_proj = numpy_model.project_onto_eofs([X1, X2], scaling=scaling)
    actual_pandas_proj = pandas_model.project_onto_eofs([df1, df2], scaling=scaling)
    actual_xarray_proj = xarray_model.project_onto_eofs([da1, da2], scaling=scaling)
