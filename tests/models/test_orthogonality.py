...
# =============================================================================
# EOF
@pytest.mark.parametrize('dim, use_coslat', [
    (('time',), True),
    (('lat', 'lon'), False),
    (('lon', 'lat'), False),
])
def test_eof_components(dim, use_coslat, mock_data_array):
    ...
    assert np.allclose(V.T @ V, np.eye(V.shape[1]), atol=1e-5), 'Components are not orthogonal'
...
def test_eof_scores(dim, use_coslat, mock_data_array):
    ...
    assert np.allclose(U.T @ U, np.eye(U.shape[1]), atol=1e-5), 'Scores are not orthogonal'
...
def test_ceof_components(dim, use_coslat, mock_data_array):
    ...
    model.fit(mock_data_array, dim=dim)
    V = model.data.components.values
    assert np.allclose(V.conj().T @ V, np.eye(V.shape[1]), atol=1e-5), 'Components are not unitary'
...
def test_ceof_scores(dim, use_coslat, mock_data_array):
    ...
    model.fit(mock_data_array, dim=dim)
    U = model.data.scores.values
    assert np.allclose(U.conj().T @ U, np.eye(U.shape[1]), atol=1e-5), 'Scores are not unitary'

# Rotated EOF
@pytest.mark.parametrize('dim, use_coslat, power', [
    (('time',), True, 1),
    (('lat', 'lon'), False, 1),
    (('lon', 'lat'), False, 1),
    (('time',), True, 2),
...
def test_reof_components(dim, use_coslat, power, mock_data_array):
    ...
    K = V.conj().T @ V
    assert np.allclose(np.diag(K), np.ones(V.shape[1]), atol=1e-5), 'Components are not normalized'
...
def test_reof_scores(dim, use_coslat, power, mock_data_array):
    ...
    if power == 1:
        ...
        assert np.allclose(K, np.eye(K.shape[1]), atol=1e-5), 'Components are not orthogonal'
    else:
...
def test_creof_scores(dim, use_coslat, power, mock_data_array):
    ...
    if power == 1:
        ...
        assert np.allclose(K, np.eye(K.shape[1]), atol=1e-5), 'Components are not unitary'
    else:
...
def test_mca_components(dim, use_coslat, mock_data_array):
    ...
    K1 = V1.T @ V1
    K2 = V2.T @ V2
    assert np.allclose(K1, np.eye(K1.shape[0]), atol=1e-5), 'Left components are not orthogonal'
    assert np.allclose(K2, np.eye(K2.shape[0]), atol=1e-5), 'Right components are not orthogonal'
...
def test_cmca_components(dim, use_coslat, mock_data_array):
    ...
    K1 = V1.conj().T @ V1
    K2 = V2.conj().T @ V2
    assert np.allclose(K1, np.eye(K1.shape[0]), atol=1e-5), 'Left components are not unitary'
    assert np.allclose(K2, np.eye(K2.shape[0]), atol=1e-5), 'Right components are not unitary'
...
# Rotated MCA
@pytest.mark.parametrize('dim, use_coslat, power, squared_loadings', [
    (('time',), True, 1, False),
...
def test_rmca_components(dim, use_coslat, power, squared_loadings, mock_data_array):
    ...
    K1 = V1.conj().T @ V1
    K2 = V2.conj().T @ V2
    assert np.allclose(np.diag(K1), np.ones(K1.shape[0]), atol=5.0e-2), 'Components are not normalized'
    assert np.allclose(np.diag(K2), np.ones(K2.shape[0]), atol=5.0e-2), 'Components are not normalized'
    # Assert that off-diagonals are not zero
...
def test_crmca_components(dim, use_coslat, power, squared_loadings, mock_data_array):
    ...
    K1 = V1.conj().T @ V1
    K2 = V2.conj().T @ V2
    assert np.allclose(np.diag(K1), np.ones(K1.shape[0]), atol=1.0e-1), 'Components are not normalized'
    assert np.allclose(np.diag(K2), np.ones(K2.shape[0]), atol=1.0e-1), 'Components are not normalized'
    # Assert that off-diagonals are not zero
    assert not np.allclose(K1, np.eye(K1.shape[0])), 'Rotated components are orthogonal'
    assert not np.allclose(K2, np.eye(K2.shape[0])), 'Rotated components are orthogonal'
...
def test_eof_transform(dim, use_coslat, mock_data_array):
    ...
    assert np.allclose(scores, pseudo_scores, atol=1e-4), 'Transformed data does not match the scores'
...
def test_reof_transform(dim, use_coslat, power, mock_data_array):
    ...
    assert np.allclose(scores, pseudo_scores, atol=1e-3), 'Transformed data does not match the scores'
...
def test_mca_transform(dim, use_coslat, mock_data_array):
    ...
    assert np.allclose(scores1, pseudo_scores1, atol=1e-4), 'Transformed data does not match the scores'
    assert np.allclose(scores2, pseudo_scores2, atol=1e-4), 'Transformed data does not match the scores'
...
def test_rmca_transform(dim, use_coslat, power, squared_loadings, mock_data_array):
    ...
    assert np.allclose(scores1, pseudo_scores1, atol=1e-5), 'Transformed data does not match the scores'
    assert np.allclose(scores2, pseudo_scores2, atol=1e-5), 'Transformed data does not match the scores'
...
def test_reof_inverse_transform(dim, use_coslat, power, mock_data_array):
    ...
    r2 = r2.mean()
    # Choose a threshold of 0.95; a bit arbitrary
    assert r2 > 0.95, f'Inverse transform does not produce a good reconstruction (R2={r2.values:.2f})'
...
def test_mca_inverse_transform(dim, use_coslat, mock_data_array):
    ...
    r2_2 = r2_2.mean()
    # Choose a threshold of 0.95; a bit arbitrary
    assert r2_1 > 0.95, f'Inverse transform does not produce a good reconstruction of left field (R2={r2_1.values:.2f})'
    assert r2_2 > 0.95, f'Inverse transform does not produce a good reconstruction of right field (R2={r2_2.values:.2f})'
...
def test_rmca_inverse_transform(dim, use_coslat, power, squared_loadings, mock_data_array):
    '''Inverse transform produces an approximate reconstruction of the original data'''
    data1 = mock_data_array.copy()
    data2 = data1.copy()**2
    ...
    assert r2_1 > 0.75, f'Inverse transform does not produce a good reconstruction of left field (R2={r2_1.values:.2f})'
    assert r2_2 > 0.75, f'Inverse transform does not produce a good reconstruction of right field (R2={r2_2.values:.2f})'
...
def test_crmca_inverse_transform(dim, use_coslat, power, squared_loadings, mock_data_array):
    '''Inverse transform produces an approximate reconstruction of the original data'''
    ...
    assert r2_1 > 0.80, f'Inverse transform does not produce a good reconstruction of left field (R2={r2_1.values:.2f})'
    assert r2_2 > 0.80, f'Inverse transform does not produce a good reconstruction of right field (R2={r2_2.values:.2f})'
