...
def test_data_array_list_stacker_fit_transform(dim_sample, dim_feature, mock_data_array_list):
    ...
    [xr.testing.assert_equal(stacked, transformed) for stacked, transformed in zip(stacked_data, transformed_data)]
    ...
    for stckr, da in zip(stacker.stackers, mock_data_array_list):
        ...
        for dim, coords in da.coords.items():
            assert stckr.coords_in_[dim].size == coords.size, 'Dimension {} has different size.'.format(dim)
        assert stckr.coords_out_['sample'].size == np.prod([coords.size for dim, coords in da.coords.items() if dim in dim_sample]), 'Sample dimension has different size.'
        assert stckr.coords_out_['feature'].size == np.prod([coords.size for dim, coords in da.coords.items() if dim in dim_feature]), 'Feature dimension has different size.'

    # Check that invalid input raises an error in transform
    with pytest.raises(ValueError):
            stacker.transform([xr.DataArray(np.random.rand(2, 4, 5), dims=("a", "y", "x")) for _ in range(3)])
