...
def test_init_params(with_std, with_coslat, with_weights):
    s = SingleDataArrayScaler(with_std=with_std, with_coslat=with_coslat, with_weights=with_weights)
...
def test_transform_params(with_std, with_coslat, with_weights, mock_data_array):
    ...
    feature_dims = ['lat', 'lon']
    size_lats = mock_data_array.lat.size
    weights = xr.DataArray(np.random.rand(size_lats), dims=['lat'], coords={'lat': mock_data_array.lat})
    s.fit(mock_data_array, sample_dims, feature_dims, weights)
    ...
    if with_std:
        ...
        if with_coslat or with_weights:
            ...
            assert np.allclose(transformed_std, 1), 'Standard deviation of the transformed data is not one.'

    if with_coslat:
        assert s.coslat_weights is not None, "Scaler coslat_weights is None."
        assert not np.array_equal(transformed, mock_data_array), "Data has not been transformed."
...
def test_fit_dims(dim_sample, dim_feature, mock_data_array):
    ...
    assert set(s.mean.dims) == set(mock_data_array.dims) - set(dim_sample), "Mean has wrong dimensions."
    assert set(s.std.dims) == set(mock_data_array.dims) - set(dim_sample), "Standard deviation has wrong dimensions."
...
def test_fit_transform_dims(dim_sample, dim_feature, mock_data_array):
    ...
    assert set(transformed.dims) == set(mock_data_array.dims), "Transformed data has wrong dimensions."
