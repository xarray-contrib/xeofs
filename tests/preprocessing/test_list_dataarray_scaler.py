...
from xeofs.preprocessing.scaler import ListDataArrayScaler
...
def test_init_params(with_std, with_coslat, with_weights):
    s = ListDataArrayScaler(with_std=with_std, with_coslat=with_coslat, with_weights=with_weights)
...
def test_fit_params(with_std, with_coslat, with_weights, mock_data_array_list):
    listscalers = ListDataArrayScaler(with_std=with_std, with_coslat=with_coslat, with_weights=with_weights)
    data = mock_data_array_list.copy()
    sample_dims = ['time']
    feature_dims = [['lat', 'lon']] * 3
    size_lats_list = [da.lat.size for da in data]
    weights = [xr.DataArray(np.random.rand(size), dims=['lat']) for size in size_lats_list]
...
def test_transform_params(with_std, with_coslat, with_weights, mock_data_array_list):
    ...
    transformed2 = listscalers.fit_transform(mock_data_array_list, sample_dims, feature_dims, weights)
    ...
    for t, t2, s, ref in zip(transformed, transformed2, listscalers.scalers, data):
        ...
        if with_std:
            ...
            if with_coslat or with_weights:
                assert (t_std <= 1).all(), 'Standard deviation of the transformed data is larger one.'
            else:
                assert np.allclose(t_std, 1), 'Standard deviation of the transformed data is not one.'

        if with_coslat:
            assert s.coslat_weights is not None, "Scaler coslat_weights is None."
            assert not np.array_equal(t, mock_data_array_list), "Data has not been transformed."

        if with_weights:
            assert s.weights is not None, "Scaler weights is None."
            assert not np.array_equal(t, ref), "Data has not been transformed."
...
def test_inverse_transform_params(with_std, with_coslat, with_weights, mock_data_array_list):
    listscalers = ListDataArrayScaler(with_std=with_std, with_coslat=with_coslat, with_weights=with_weights)
    data = mock_data_array_list.copy()
    sample_dims = ['time']
    feature_dims = [['lat', 'lon']] * 3
    ...
    listscalers.fit(mock_data_array_list, sample_dims, feature_dims, weights)
    transformed = listscalers.transform(mock_data_array_list)
    inverted = listscalers.inverse_transform(transformed)
...
def test_fit_dims(dim_sample, dim_feature, mock_data_array_list):
    listscalers = ListDataArrayScaler(with_std=True)
    data = mock_data_array_list.copy()
    ...
    for s in listscalers.scalers:
        ...
        assert set(s.mean.dims) == set(mock_data_array_list.dims) - set(dim_sample), "Mean has wrong dimensions."
        assert set(s.std.dims) == set(mock_data_array_list.dims) - set(dim_sample), "Standard deviation has wrong dimensions."
...
def test_fit_transform_dims(dim_sample, dim_feature, mock_data_array_list):
    ...
    data = mock_data_array_list.copy()
    dim_feature = [dim_feature] * 3
    transformed = listscalers.fit_transform(mock_data_array_list, dim_sample, dim_feature)
...
def test_fit_input_type(dim_sample, dim_feature, mock_data_array, mock_dataset, mock_data_array_list):
    s = ListDataArrayScaler()
    dim_feature = [dim_feature] * 3
