...
class CCA(CPCCA):
    ...
    CCA seeks to find paris of coupled patterns that maximize the correlation [1]_ [2]_ .
    ...
    Perform CCA on two datasets on a regular longitude-latitude grid:

    >>> model = CCA(n_modes=5, use_coslat=True)
    ...
    def __init__(
        ...
        self.attrs.update({"model": "Canonical Correlation Analysis"})
        # Renove alpha from the inherited CPCCA serialization params because it is hard-coded for CCA
        self._params.pop("alpha")


class ComplexCCA(ComplexCPCCA, CCA):
    """Complex CCA.

    CCA applied to a complex-valued field obtained from a pair of variables such
    as the zonal and meridional components, :math:`U` and :math:`V`, of the wind
    field. Complex CCA analysis then maximizes the correlation between
    ...
    and fit the Complex CCA model:

    >>> model = ComplexCCA(n_modes=5)
    >>> model.fit(X, Y, "time")
    ...
    def __init__(
        ...
        self.attrs.update({"model": "Complex CCA"})
        # Renove alpha from the inherited CPCCA serialization params because it is hard-coded for CCA
        self._params.pop("alpha")


class HilbertCCA(HilbertCPCCA, ComplexCCA):
    """Hilbert CCA.

    Hilbert CCA  extends CCA by examining amplitude-phase relationships. It
    ...
    def __init__(
        ...
        self.attrs.update({"model": "Hilbert CCA"})
        # Renove alpha from the inherited CPCCA serialization params because it is hard-coded for CCA
        self._params.pop("alpha")
