...
class EOFRotator(EOF):
    ...
    def fit(self, model) -> Self:
        """Rotate the solution obtained from ``xe.single.EOF``.
...
class ComplexEOFRotator(EOFRotator, ComplexEOF):
    ...
    Examples
    --------
    >>> model = xe.single.ComplexEOF(n_modes=10)
    >>> model.fit(data)
    >>> rotator = xe.single.ComplexEOFRotator(n_modes=10)
    >>> rotator.fit(model)
    >>> rotator.components()
...
class HilbertEOFRotator(EOFRotator, HilbertEOF):
    ...
    Examples
    --------
    >>> model = xe.single.HilbertEOF(n_modes=10)
    >>> model.fit(data)
    >>> rotator = xe.single.HilbertEOFRotator(n_modes=10)
    >>> rotator.fit(model)
    >>> rotator.components()
